 ...
 * Twitter class.
 *
 * 	Code is originally created by Tijs Verkoyen <php-twitter@verkoyen.eu>, it has been extended to fit with the Whip Framework
 * 
...
class Twitter extends UncachedWhipPlugin  {
	// Error Messages
	const E_TWEET_MISSING 			= 'The status message for this tweet is missing';
	const E_HTTP_METHOD_INCORRECT 	= 'The following HTTP Method is inccorect: ';
...
// class methods
	...
	private function doOAuthCall($method, array $parameters = null)
	{
		...
		// error?
		if($errorNumber != '') throw new WhipException($errorMessage, $errorNumber);
	...
	private function doCall($url, array $parameters = null, $authenticate = false, $method = 'GET', $filePath = null, $expectJSON = true, $returnHeaders = false)
	{
		...
		// validate method
		if(!in_array($method, $allowedMethods)) throw new WhipException('Unknown method ('. $method .'). Allowed methods are: '. implode(', ', $allowedMethods));
		...
		if($json === null)
		{
			...
			// throw exception
			throw new WhipException('Invalid response.');
		}
		...
		if(isset($json['errors']))
		{
			...
			// throw exception
			if(isset($json['errors'][0]['message'])) throw new WhipException($json['errors'][0]['message']);
			else throw new WhipException('Invalid response.');
		}
		...
		if(isset($json['error']))
		{
			...
			// throw exception
			throw new WhipException($json['error']);
		}
	...
	private function getConsumerKey()
	{
		if ( empty($this->consumerKey) )
			$this->consumerKey = $this->_config['consumer_key'];
	...
	private function getConsumerSecret()
	{
		if ( empty($this->consumerSecret) )
			$this->consumerSecret = $this->_config['consumer_secret'];
	...
	private function getOAuthToken()
	{
		if ( empty($this->oAuthToken) )
			$this->oAuthToken = $this->_config['oauth_token'];
	...
	{
		if ( empty($this->oAuthTokenSecret) )
			$this->oAuthTokenSecret = $this->_config['oauth_token_secret'];
...
// Timeline resources
	...
	{
		// validate
		if($count != null && $count > 200) throw new WhipException('Count may not be greater than 200.');
...
// Tweets resources
	...
	public function statusesRetweets($id, $count = null)
	{
		...
		if($count != null && $count > 100) {
            throw new WhipException('Count may not be greater than 100.');
            return false;
		}
	...
	public function statusesIdRetweetedBy($id, $count = null, $page = null)
	{
		...
		if($count != null && $count > 200) {
			throw new WhipException('Count may not be greater than 200.');
            return false;
		}
...
// User resources
	...
	public function usersShow($id = null, $userId = null, $screenName = null)
	{
		...
		if($id == '' && $userId == '' && $screenName == '') {
			throw new WhipException('Specify an id or an userId or a screenName.');
            return false;
		}
	...
	public function usersProfileImage($screenName, $size = 'normal')
	{
		...
		if($size != null && !in_array($size, $allowedSizes)) {
			throw new WhipException('Invalid size ('. $size .'), possible values are: '. implode($allowedSizes) .'.');
            return false;
		}
...
// List resources
	...
	public function userListsCreate($user, $name, $mode = null, $description = null)
	{
		...
		if($mode != null && !in_array($mode, $allowedModes)) {
			throw new WhipException('Invalid mode (), possible values are: '. implode($allowedModes) .'.');
            return false;
		}
...
// Friendship resources
	...
	public function friendshipsShow($sourceId = null, $sourceScreenName = null, $targetId = null, $targetScreenName = null)
	{
		...
		if($sourceId == '' && $sourceScreenName == '') {
			throw new WhipException('Specify an sourceId or a sourceScreenName.');
            return false;
		}
		if($targetId == '' && $targetScreenName == '') {
			throw new WhipException('Specify an targetId or a targetScreenName.');
            return false;
		}
...
// Account resources
	...
	public function accountUpdateProfileColors($profileBackgroundColor = null, $profileTextColor = null, $profileLinkColor = null, $profileSidebarFillColor = null, $profileSidebarBorderColor = null)
	{
		...
		if($profileBackgroundColor == '' && $profileTextColor == '' && $profileLinkColor == '' && $profileSidebarFillColor == '' && $profileSidebarBorderColor == '') {
			throw new WhipException('Specify a profileBackgroundColor, profileTextColor, profileLinkColor, profileSidebarFillColor or a profileSidebarBorderColor.');
            return false;	
		}
	...
	public function accountUpdateProfileImage($image)
	{
		...
		if(!file_exists($image)) {
			throw new WhipException('Image ('. $image .') doesn\'t exists.');
            return false;
		}
...
// Spam Reporting resources
	...
	{
		// validate
		if($id == '' && $userId == '' && $screenName == '') throw new WhipException('Specify an id or an userId or a screenName.');
...
// OAuth resources
	...
	public function oAuthRequestToken($callbackURL = null)
	{
		...
		// validate
		if(!isset($response['oauth_token'], $response['oauth_token_secret'])) throw new WhipException(implode(', ', array_keys($response)));
	...
	{
		throw new WhipException('Not implemented');
